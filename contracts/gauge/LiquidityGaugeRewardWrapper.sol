...
*    function rewarded_token() -> address view
*    function deposit(_value uint256) nonpayable
*    function withdraw(_value uint256) nonpayable
*    function claimable_reward(addr address) -> uint256 view
*    function claimed_rewards_for(addr address) -> uint256 view
*    function claimable_tokens(addr address) -> uint256 nonpayable
*    function claim_rewards() nonpayable
*
*
*interface Minter
...
function __init__( bytes64 _name, bytes32 _symbol, address _gauge, address _admin)external {
    /**
    ...
    */
    ...
    rewarded_token = LiquidityGauge(_gauge).rewarded_token();
    lp_token = lp_token;
    gauge = _gauge;
    admin = _admin;
}
...
function _checkpoint(address addr)internal{
    address gauge = gauge;
    address token = crv_token;
    uint256 total_balance = totalSupply;

    uint256 d_reward = ERC20(token).balanceOf(address(this));
    Minter(minter).mint(gauge);
    d_reward = ERC20(token).balanceOf(address(this)) - d_reward;
    ...
    d_reward = ERC20(token).balanceOf(address(this));
    ...
    I = reward_integral + dI;
    reward_integral = I;
    claimable_rewards[addr] += balanceOf[addr] * (I - reward_integral_for[addr]) / 10 ** 18;
...
function claimable_reward(address addr)external view returns (uint256){
    ...
    */
    address gauge = gauge;
    uint256 d_reward = LiquidityGauge(gauge).claimable_reward(address(this)) - LiquidityGauge(gauge).claimed_rewards_for(address(this));
    ...
    return claimable_rewards[addr] + balanceOf[addr] * (I - reward_integral_for[addr]) / 10 ** 18;
}
...
function claim_tokens(address addr = msg.sender)external{
    ...
    */
    _checkpoint(addr);
    assert (ERC20(crv_token).transfer(addr, claimable_crv[addr]));
    assert (ERC20(rewarded_token).transfer(addr, claimable_rewards[addr]));

    claimable_crv[addr] = 0;
    claimable_rewards[addr] = 0;
}
